...
var StudioMonitor = /** @class */ (function () {
    function StudioMonitor(ipAddress, port, ready) {
        ...
        this.updateConfigData()
            ...
            _this_1.updateSourcesData()
                .then(function () { return ready(); })["catch"](function (error) {
                return ready(new Error("Unable to find StudioMonitor at " + _this_1.ip + ":" + _this_1.port + ". Error: " + error.message));
            });
    ...
    StudioMonitor.prototype.updateConfigData = function () {
        var _this = this;
        return new Promise(function (resolve, reject) {
            axios_1["default"]
    ...
    StudioMonitor.prototype.getCurrentSource = function () {
        var _this_1 = this;
        return new Promise(function (resolve, reject) {
            _this_1.updateConfigData()
                .then(function () {
                if (_this_1.config != undefined)
                    resolve(_this_1.config.NDI_source);
                else
                    reject(new Error('Config seems invalid'));
            })["catch"](function () { return reject(new Error('Unable to get current StudioMonitor config')); });
        });
    };
